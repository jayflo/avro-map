...
function map(schema, cb, initialValueFn) {
  ...
  return schema.map(function(entry) {
    ...
    var typeChain = [];
    var namespace = '';
    var initialValue = initialValueFn();

    return reduceEntry(
      cb, registry, namespace, typeChain, initialValue, result, entry
    );
  });
}

function TypeEntryPair(type, entry) {
  return {type: type, ref: entry};
}
...
function reduceEntry(
  ...
  if (isPrimitiveEntry(entryOrName)) {
    tep = TypeEntryPair(entryOrName);
  } else if (isUnionEntry(entryOrName)) {
    tep = TypeEntryPair(COMPLEX_TYPE_NAMES.UNION, entryOrName);
  } else if (isArrayEntry(entryOrName) || isMapEntry(entryOrName)) {
    tep = TypeEntryPair(entryOrName.type, entryOrName);
  } else {
    if (isRegisterable(entryOrName)) {
      nameDescriptor = registry.add(
        namespace, entryOrName, TypeEntryPair(entryOrName.type, entryOrName)
      );
  ...
  (recursables || getRecursables(tep)).forEach(function(r) {
